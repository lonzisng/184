...
module.exports ={
    complex(callback){
        let sql="SELECT goods.goods_id as id,goods.goods_name as name,goods_att.price,goodspic.route as src FROM goods JOIN goodspic ON goodsPic.goods_id = goods.goods_id JOIN goods_att ON goods.goods_id = goods_att.goods_id WHERE goods.state=1 AND goodspic.master=1 AND goods_att.order_id=1 ORDER BY  goods.burst DESC , goods.volume DESC";
        db.sqlpool(sql,[],callback);
    },
    new(callback){
        let sql="SELECT goods.goods_id as id,goods.goods_name as name,goods_att.price,goodspic.route as src FROM goods JOIN goodspic ON goodsPic.goods_id = goods.goods_id JOIN goods_att ON goods.goods_id = goods_att.goods_id WHERE goods.state=1 AND goodspic.master=1 AND goods_att.order_id=1 ORDER BY goods.create_time DESC";
        db.sqlpool(sql,[],callback);
    },
    volume(callback){
        let sql="SELECT goods.goods_id as id,goods.goods_name as name,goods_att.price,goodspic.route as src FROM goods JOIN goodspic ON goodsPic.goods_id = goods.goods_id JOIN goods_att ON goods.goods_id = goods_att.goods_id WHERE goods.state=1 AND goodspic.master=1 AND goods_att.order_id=1 ORDER BY goods.volume DESC";
        db.sqlpool(sql,[],callback);
    },
